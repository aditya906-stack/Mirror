...
type FeedbackData = {
  ...
  behaviors: { id: string; text: string; category: string }[];
...
export function FeedbackView({ token }: { token: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "This link could not be loaded."
        );
      } finally {
        setLoading(false);
      }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setDone(true);
      toast.success("Thank you. Your observations have been recorded.");
    } catch (err) {
      ...
      <Shell>
        <div className="mx-auto max-w-lg text-center">
          ...
          <p className="mt-6 text-base leading-relaxed text-ink-soft">
            Your observations of {data.invitation.subjectName} have been
            recorded. They will appear in {data.invitation.subjectName}’s Mirror
            report as part of the {data.invitation.circle} circle —
            confidentially. Your individual identity will not be shown.
          </p>
          <p className="mt-8 font-display text-lg italic text-ink-soft">
            You have offered an honest reflection. That is a quiet gift.
          </p>
        </div>
      </Shell>
    );
      ...
      <div className="mx-auto max-w-3xl px-5 py-12 sm:px-8 sm:py-16">
        ...
        <div className="mb-12 border-b border-line pb-10">
          <span className="font-mono text-xs uppercase tracking-widest text-ink-faint">
            Confidential observation
          </span>
          <h1 className="mt-4 font-display text-4xl leading-tight text-ink sm:text-5xl">
            How do you experience {data.invitation.subjectName}?
          </h1>
          <p className="mt-4 max-w-xl text-base leading-relaxed text-ink-soft">
            You have been invited to observe {data.invitation.subjectName} as
            part of their{" "}
            <span className="font-display text-ink">
              {data.invitation.circle}
            </span>{" "}
            circle. Answer honestly, based on what you have actually witnessed.
          </p>
          ...
          <div className="mt-6 border border-line bg-surface px-5 py-4">
            <p className="text-sm leading-relaxed text-ink-soft">
              ...
              {data.invitation.subjectName} will see that this feedback came
              from their {data.invitation.circle} circle, but will never see
              that it came from you specifically. Your honesty is protected.
            </p>
          </div>

          <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2 py-2">
          ...
          <div className="mx-auto flex max-w-3xl items-center justify-between gap-4">
            ...
            >
              <span className="font-display text-base">
                {submitting ? "Submitting…" : "Submit in confidence"}
              </span>
            </button>
          </div>
        </div>
